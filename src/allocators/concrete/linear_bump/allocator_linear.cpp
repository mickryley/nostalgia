...
namespace nostalgia::linear {
	...
	LinearAllocator g_linear_allocator(buffer, buffer_size, "Global");

	// === Singleton Static Allocator ===
    LinearAllocator& SingletonLinearAllocator::get_instance() {
        static LinearAllocator s_linear_allocator(buffer, buffer_size, "Singleton Static");
        return s_linear_allocator;
    }

    // === Base Class Constructors ===
    LinearAllocator::LinearAllocator(std::byte* buf, size_t cap, const char* caller)
    : m_buffer(buf), m_capacity(cap), m_offset(0), m_peakCapacity(0) {

        log::print("[{}] LinearAllocator: Initialised from location [{}] with a capacity of [{}] bytes", caller, (void*)m_buffer, m_capacity);
    }
